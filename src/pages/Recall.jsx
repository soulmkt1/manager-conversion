...
const COLUMNS = [
  ...
  { key: 'manager', label: '실장', w: 90 },
  { key: 'customer_name', label: '고객명', w: 320 },
]

export default function Recall({ data, onChange }) {
  const [rows, setRows] = useState([])
  const [mgrFilter, setMgrFilter] = useState('')
  const [q, setQ] = useState('')
  const [dateFilter, setDateFilter] = useState({ from: '', to: '' })
  const [selected, setSelected] = useState(() => new Set())
  useEffect(() => setRows(data.recall || []), [data.recall])

  const managers = useMemo(() => [...new Set((data.recall || []).map((r) => r.manager).filter(Boolean))].sort(), [data.recall])
  const allDates = useMemo(() => (data.recall || []).map((r) => r.report_date), [data.recall])

  const filtered = useMemo(() => (rows || [])
    .filter((r) => {
      if (mgrFilter && r.manager !== mgrFilter) return false
      if (!matchDate(r.report_date, dateFilter)) return false
      if (q && !COLUMNS.map((c) => r[c.key]).join(' ').toLowerCase().includes(q.toLowerCase())) return false
      return true
    })
  ...
  async function persist(id, key, value) {
    ...
    setSaveState('saving')
    try {
      await updateRow('recall', id, { [key]: value === '' ? null : value })
      setSaveState('saved')
  ...
  async function bulkRemove() {
    ...
    try {
      await deleteMany('recall', selectedIds)
  ...
  return (
    <div className="page">
      <h2>리콜 <span className="muted">({(data.recall || []).length}건)</span></h2>
      <div className="toolbar">
        ...
        <button className="btn ghost" onClick={() => downloadCSV(`리콜_${new Date().toISOString().slice(0, 10)}.csv`, toCSV(filtered, COLUMNS))}>⬇ CSV</button>
      </div>
